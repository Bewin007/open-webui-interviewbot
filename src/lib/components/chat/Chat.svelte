				...
				<div class="flex flex-col flex-auto z-10 w-full">
					{#if $settings?.landingPageMode === 'chat' || createMessagesList(history.currentId).length > 0}
						...
						<div class=" pb-[1.6rem]">
							...
							/>
							<!-- <div class=" text-gray-500 text-center line-clamp-1 right-0 left-0">
							
							<button on:click={() => submitPrompt('Start interview !')} >Start Interview ⏩</button>
							<button on:click={() => submitPrompt('skip this question')}>Skip🔄</button>

							<button on:click={() => submitPrompt('Generate the Expected Answer for this question')}>Generate Answer💡</button>

							<button on:click={() => submitPrompt('Provide Feedback')}>Provide Feedback📋</button>
							<button on:click={() => submitPrompt('End interview')}>End Interview 🔚</button>

							</div> -->
							<div class="text-center flex justify-center space-x-4 text-gray-500">
  <button 
    class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded" 
    on:click={() => submitPrompt('Start interview !')}>
    Start Interview ⏩
  </button>

  <button 
    class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded" 
    on:click={() => submitPrompt('skip this question')}>
    Skip 🔄
  </button>

  <button 
    class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded" 
    on:click={() => submitPrompt('Generate the Expected Answer for this question')}>
    Generate Answer 💡
  </button>

  <button 
    class="bg-purple-500 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded" 
    on:click={() => submitPrompt('Provide Feedback')}>
    Provide Feedback 📋
  </button>

  <button 
    class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded" 
    on:click={() => submitPrompt('End interview')}>
    End Interview 🔚
  </button>
</div>
